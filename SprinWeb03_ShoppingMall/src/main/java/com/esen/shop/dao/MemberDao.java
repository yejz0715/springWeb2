...
public class MemberDao {
	...
	// 8.  List 가 아닌 단일  Dto 리턴되어 야 한다면  list.get(0) 을 리턴합니다. 
	//      단, 리턴값이 null 경우 if 로 처리합니다
	...
	public void insertMember(MemberVO mvo) {
		String sql = "insert into member(id, pwd, name, zip_num, address, email, phone) "
				+ " values(? , ? , ? , ? , ? , ? , ? )";
		template.update( sql, mvo.getId(), mvo.getPwd(), mvo.getName(),  mvo.getZip_num(), 
				mvo.getAddress(),	mvo.getEmail(), mvo.getPhone() );
	}
